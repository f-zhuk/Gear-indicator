...
void putChar(uint8_t character, font_t font, uint8_t color) // color used only for 1bpp fonts
{
    uint8_t cmap = 0;
    ...
    while (i<box_h)
    {
        ...
        while (j<box_w)
        {
            ...
            {
                case 4:
                data = (bm[bmi+((i*font.glyph_dsc[character].box_w+j)>>1)] >> ((~(i*font.glyph_dsc[character].box_w+j)&0x01)<<2)) & 0x0F;
                break;

                case 1:
                if ((bm[bmi+((i*font.glyph_dsc[character].box_w+j)>>3)] >> (~(i*font.glyph_dsc[character].box_w+j)&0x07)) & 0x01)
                    data = color&0x0F;
                break;
            }
            ...
            if(data)
            {
                ...
                display.target_buffer[wbi>>1] |=  (data << ((~wbi&0x01)<<2));
            }
            j++;
        }
        i++;
...
void putString(uint8_t* string, font_t font, uint8_t color) // color used only for 1bpp fonts
{
    while (*string != 0)
    {
